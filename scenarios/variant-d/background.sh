#!/bin/bash
# Variant D: Service selector mismatch — pods are Running but Service has no endpoints.
# The Deployment pods have label app:mobility-api.
# The Service selector was changed to app:mobility-backend during a refactor, so
# the Service resolves no endpoints and all traffic returns 503.
...
# ─── Deploy Broken State ──────────────────────────────────────────────────────
# Pods run fine with label app=mobility-api.
# Service selector uses app=mobility-backend — intentional mismatch, zero endpoints.
# This is the trickiest variant: kubectl get pods looks perfectly healthy.
kubectl apply -f - <<'EOF'
apiVersion: v1
kind: Namespace
metadata:
...
metadata:
  ...
  namespace: mobility
  annotations:
    kubernetes.io/change-cause: "updated to v1.4.2"
spec:
  replicas: 2
  selector:
    matchLabels:
...
# Wait for pods to reach Running/Ready state — the pods ARE healthy, the service is broken
kubectl rollout status deployment/mobility-api -n mobility --timeout=90s || true
